...
function setFirstMove(who) {
  ...
  document.querySelectorAll(".firstMoveOption").forEach(btn => {
    ...
    if (
      (who === "player" && value === playerName.toLowerCase()) ||
      (who === "ai" && value === aiName.toLowerCase()) ||
      (who === "random" && value === "zufällig")
    ) {
      btn.classList.add("selected");
    }
  });
...
function setDrawRule(rule) {
  ...
  document.querySelectorAll(".drawRuleOption").forEach(btn => {
    btn.classList.remove("selected");
    if (
      (rule === "draw" && btn.textContent.includes("Unentschieden")) ||
      (rule === "winner" && btn.textContent.includes("nachzieht"))
    ) {
      btn.classList.add("selected");
    }
  });
...
function startGame() {
  ...
  firstMoveButtons[0].textContent = playerName;
  firstMoveButtons[1].textContent = aiName;
  firstMoveButtons[2].textContent = "Zufällig";

  // Regeltexte aktualisieren
  document.querySelectorAll(".drawRuleOption")[0].textContent = "Unentschieden möglich";
  document.querySelectorAll(".drawRuleOption")[1].textContent = "Wer nachzieht, gewinnt bei Gleichstand";
...
function checkScore(currentSymbol) {
  ...
  for (let r = 0; r < gridSize; r++) {
    for (let c = 0; c <= gridSize - 3; c++) {
      ...
      if (isTripleMatch(i1, i2, i3, currentSymbol)) {
        board[i1] = "BLOCK";
        board[i2] = "BLOCK";
        board[i3] = "BLOCK";
        scored = true;
